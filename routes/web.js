...
router.get('/apps', async function (req, res) {
	...
	// Get app versions
	requestString = '/apps/versions?developerId=' + config.DEVELOPER_ID + '&query=' + encodeURIComponent('{$or: [{"status.value":"rejected",isLatestVersion:true},{isLive:true},{"status.value":{$in:["inDevelopment","inReview","pending"]}}]}');
	...
	requestString = '/stats/series/month/views?query=' + encodeURIComponent("{developerId: '" + config.DEVELOPER_ID + "'}");
...
router.get('/', async function (req, res) {
	...
	requestString = '/apps?userId=' + config.USER_ID + '&query=' + encodeURIComponent('{"status.value":"approved"}') + '&sort=' + encodeURIComponent('{"randomize":1}');
	...
	requestString = '/apps?userId=' + config.USER_ID + '&query=' + encodeURIComponent('{"status.value":"approved","attributes.featured":"yes"}') + '&sort=' + encodeURIComponent('{"randomize":1}');
...
router.get('/details/:safeName', async function (req, res) {
	...
	// main app request
	requestString = '/apps/bySafeName/' + req.params.safeName + '?userId=' + config.USER_ID + '&trackViews=true';
...
router.get('/details/:appId/:version', async function (req, res) {
	let requestString, app, relatedApps, category, appid, user;
	// main app request
	requestString = '/apps/' + req.params.appId + '/versions/' + req.params.version + '?developerId=' + config.DEVELOPER_ID;
	app = await openChannelRequest(requestString, req, res);
	if (app.status === 'error') {
		return res.render('app/error', {error: app});
	}
...
router.get('/web.js/category-filter?', async function (req, res) {
	...
	reqstring = '{"status.value":"approved","customData.category":"' + req.query.category + '"}';
	// request for apps in a category
	requestString = '/apps?userId=' + config.USER_ID + '&query=' + encodeURIComponent(reqstring) + '&sort=' + encodeURIComponent('{"randomize":1}');
...
router.get('/web.js/collection-filter?', async function (req, res) {
	...
	} else if (req.query.collection === 'My Apps') { // My Apps
		reqstring = '{"status.value":"approved"}';
		owner = '&isOwner=true';
	} else if (req.query.collection === 'Popular') { // popular apps
		reqstring = '{"status.value":"approved"}';
		sort = '&sort=' + encodeURIComponent('{"statistics.views.30day" : -1}');
	} else if (req.query.collection === 'Featured') { // featured apps
		reqstring = '{"status.value":"approved","attributes.featured":"yes"}';
		sort = '&sort=' + encodeURIComponent('{"randomize":1}');
	}
	requestString = '/apps?userId=' + config.USER_ID + '&query=' + encodeURIComponent(reqstring) + sort + owner;
	apps = await openChannelRequest(requestString, req, res);
	if (apps.status === 'error') {
		return res.render('app/error', {error: apps});
	}
...
router.get('/web.js/search?', async function (req, res) {
	...
	if (req.query.set === 'category') {
		reqstring = '{"status.value":"approved","customData.category":"' + req.query.category + '"}';
		sort = '&sort=' + encodeURIComponent('{"randomize":1}');
	} else if (req.query.set === 'collection') { // search apps in collections
		if (req.query.collection === 'All Apps') {
			reqstring = '{"status.value":"approved"}';
		} else if (req.query.collection === 'My Apps') { // search within my app
			reqstring = '{"status.value":"approved"}';
			owner = '&isOwner=true';
		} else if (req.query.collection === 'Popular') { // search within popular apps
			reqstring = '{"status.value":"approved"}';
			sort = '&sort=' + encodeURIComponent('{"statistics.views.30day" : -1}');
		} else if (req.query.collection === 'Featured') { // search within featured apps
			reqstring = '{"status.value":"approved","attributes.featured":"yes"}';
			sort = '&sort=' + encodeURIComponent('{"randomize":1}');
		}
	} else if (req.query.set === '') { // search apps
		reqstring = '{"status.value":"approved"}';
		sort = '&sort=' + encodeURIComponent('{"randomize":1}');
	}
	if (searchText === '') {
		...
		requestString = '/apps/textSearch?userId=' + config.USER_ID + '&query=' + encodeURIComponent(reqstring) + '&text=' + encodeURIComponent(searchText) + '&fields=' + encodeURIComponent(fields) + sort + owner;
		apps = await openChannelRequest(requestString, req, res);
		if (apps.status === 'error') {
			return res.render('app/error', {error: apps});
		}
...
router.get('/apps/create', function (req, res) {
	...
	res.render('app/app-create', { toast_type: toastType, toast_message: toastMessage });
});

// Edit page
router.get('/apps/update/:id/:version', async function (req, res) {
	let requestString, app, statistics, views, toastType, toastMessage;
	requestString = '/apps/' + req.params.id + '/versions/' + req.params.version + '?developerId=' + config.DEVELOPER_ID;
	app = await openChannelRequest(requestString, req, res);
	if (app.status === 'error') {
		return res.render('app/error', {error: app});
	}
	try {
		...
		if (app.customData.files) {
			app.customData.fileList = app.customData.files.toString().split(',').filter(function (el) {
				return el.length !== 0
			});
		}
		if (app.customData.images) {
			app.customData.imageList = app.customData.images.toString().split(',').filter(function (el) {
				return el.length !== 0
			});
		}
		// Get app statistics after retrieving app version
		requestString = '/stats/series/month/views?query=' + encodeURIComponent("{appId: '" + app.appId + "', developerId: '" + config.DEVELOPER_ID + "'}");
		statistics = await openChannelRequest(requestString, req, res);
		if (statistics.status === 'error') {
			return res.render('app/error', {error: statistics});
		}
	...
	res.render('app/app-edit', { app: app, statistics: statistics, toast_type: toastType, toast_message: toastMessage, views: views });
...
async function openChannelRequest(requestString, req, res) {
	...
	try {
		...
		if(!e.statusCode) {
			return {status: 'error', code: 500, errorMessage: 'internal server error'};
		}
		let error = JSON.parse(e.error);
		return {status: 'error', code: e.statusCode, errorMessage: error.errors[0].message};
	}
	return responseJson;
}
