...
router.post('/apps/create', async function (req, res) {
	...
	if (publish === 'true') {
		...
		let publishResponse = await openChannelRequest(requestString, 'POST', publishBody, req, res);
		...
		res.redirect('/apps');
	} else { // App will be published later
		// Display success message
...
router.post('/apps/publish', async function (req, res) {
	...
	publishResponse = await openChannelRequest(requestString, 'POST', body, req, res);
...
router.post('/apps/delete', async function (req, res) {
	...
	if (typeof req.body.version !== 'undefined') {
		requestString = '/apps/' + req.body.appId + '/versions/' + req.body.version + '?developerId=' + config.DEVELOPER_ID;
		deleteResponse = await openChannelRequest(requestString, 'DELETE', 'nobody', req, res);

	} else { // If version is not set, delete all app versions
		requestString = '/apps/' + req.body.appId + '?developerId=' + config.DEVELOPER_ID;
		deleteResponse = await openChannelRequest(requestString, 'DELETE', 'nobody', req, res);
...
router.post('/apps/status', async function (req, res) {
	...
	statusResponse = await openChannelRequest(requestString, 'POST', body, req, res);
	if(!statusResponse) {
...
router.post('/details', async function (req, res) {
	...
	// uninstall app code
	if (req.body.appStatus === 'active' || req.body.appStatus === 'newapp active') {
		let ownershipid;
		...
		// uninstall request
		uninstallResponse = await openChannelRequest(requestString, 'POST', uninstallBody, req, res);
	...
	} else if (req.body.appStatus === 'uninstalled' || req.body.appStatus === 'newapp' || req.body.appStatus === 'newapp uninstalled') { // install app code
		requestString = '/ownership/install';
		// install request
		installResponse = await openChannelRequest(requestString, 'POST', installBody, req, res);
		// If error is retrieved, display error message and return
		if (installResponse.status === 'error') {
			res.send('error installing');
			return;
		}
		if (req.body.appStatus === 'newapp' || req.body.appStatus === 'newapp uninstalled') {
